#!/usr/bin/env python3
"""V14 joint spatial-memory / Action Expert LoRA pilot.

Same observation-only V13 tail bank and fixed query/noise plan. Learned visual
encoding and retrieval now train jointly with the initial archive AE adapters.
Original VLM/AE/base weights, archive memory and CVOM remain frozen. READ-off
uses the SAME newly adapted expert, not the original archive expert. Primary
rollout uses the fixed final step; offline MAE-best is diagnostic only.
All original files are immutable; exact resume requires a new output directory.
"""
...
from gr00t.long_memory.expert_v4 import (LoRAConfig, ExpertLoRALinear, expert_parameters,
    expert_episode_flow_loss, install_expert_lora, set_expert_trainable)
...
from run_scripts.robomme.checkpoint_visual_expert_v14 import (
    ...
    verify_frozen_references, optimizer_parameter_names, EXPECTED_REAL_EXPERT_TARGETS,
)
...
OBJECTIVE = {"flow_weight": 1., "generated_auxiliary_weight": 0., "selection_metric": SELECTION,
             "trainable_scope": "visual_and_expert_lora",
             "frozen_original_base": True, "frozen_archive": True,
             "rollout_selection": "fixed_final_step", "mae_best_is_diagnostic": True}


def source_identity():
    names = ("train_visual_expert_v14.py", "checkpoint_visual_expert_v14.py",
             "train_demo_tail_v13.py", "checkpoint_demo_tail_v13.py",
...
def joint_parameters(visual, head):
    """Only these FP32 master tensors are owned by the optimizer."""
    return list(visual.parameters()) + list(expert_parameters(head))


def optimizer_groups(args, visual, head):
    named = sorted(visual.named_parameters())
    allowed = {id(p) for p in expert_parameters(head)}
    expert = sorted((n, p) for n, p in head.named_parameters() if id(p) in allowed)
    groups = [
        {"params": [p for _, p in entries], "param_names": ["visual." + n for n, _ in entries],
    ...
    if not expert or len(expert) != len(allowed):
        raise ValueError("Missing/aliased installed expert LoRA tensors")
    groups.append({"params": [p for _, p in expert],
        "param_names": ["expert." + n for n, _ in expert], "kind": "expert",
        "name": "expert", "lr": args.expert_learning_rate, "weight_decay": args.weight_decay})
    return groups


def frozen_guard(parent, head, cvom):
    allowed = {id(p) for p in expert_parameters(head)}
    return [(label + "." + name, value, value._version)
            for label, module in (("archive", parent), ("head", head), ("cvom", cvom))
            for name, value in list(module.named_parameters()) + list(module.named_buffers())
            if not (module is head and id(value) in allowed)]


def assert_scope(visual, parent, head, cvom, guard):
    if any(module.training for module in (parent, head, cvom)):
        raise RuntimeError("Archive/AE/CVOM must remain eval; LoRA training does not enable dropout")
    allowed = {id(p) for p in expert_parameters(head)}
    if not allowed or any(not m.enabled for m in head.modules() if isinstance(m, ExpertLoRALinear)):
        raise RuntimeError("All installed expert adapters must be enabled")
    if any(p.requires_grad or p.grad is not None for module in (parent, cvom) for p in module.parameters()):
        raise RuntimeError("Archive/CVOM must remain frozen")
    if any((p.requires_grad or p.grad is not None) and id(p) not in allowed for p in head.parameters()):
        raise RuntimeError("Original Action Expert weights must remain frozen")
    if not all(p.requires_grad and p.dtype == torch.float32 for p in joint_parameters(visual, head)):
        raise RuntimeError("All visual and expert LoRA master tensors must remain trainable FP32")
    if any(value._version != version for _, value, version in guard):
        raise RuntimeError("Original base/archive/CVOM tensor changed")
...
def parser():
    ...
    source.add_argument("--init-checkpoint", help="Original V7 archive1250 initialization, never rewritten")
    source.add_argument("--resume", help="Exact V14 resume into a NEW directory; frozen base+parent required")
    p.add_argument("--include-tail", action=argparse.BooleanOptionalAction, default=True,
                   help="V14 pilot fixes tail inclusion; --no-include-tail is rejected")
...
def parse_args(argv=None):
    ...
    if args.resume:
        ...
        if info["config"].get("trainer_variant") != DRIVER:
            raise ValueError("Exact resume requires a visual_expert_v14 checkpoint")
    ...
    args.driver_variant, args.stage, args.mode = DRIVER, 1, "visual_expert"
...
def validate_options(args):
    ...
    if args.include_tail is not True or args.replay_encoding != "framewise":
        raise ValueError("V14 requires include_tail=True and framewise replay")
    ...
    for key in ("visual_learning_rate", "expert_learning_rate", "max_grad_norm", "weight_decay"):
...
def query_objective(args, visual, parent, head, episode, item, sidecars):
    ...
    if not bool(torch.isfinite(loss)) or not loss.requires_grad:
        raise FloatingPointError("Nonfinite/disconnected joint visual/expert training objective")
...
def resume_options(args, info, sources):
    if info["config"].get("objective") != OBJECTIVE or info["metadata"].get("selection_metric") != SELECTION:
        raise ValueError("Exact V14 objective/selection semantics changed")
...
def main(argv=None):
    ...
    parent_record = initial["metadata"]["initial_parent"] if args.resume else parent_reference(base, source)
    ...
    if args.resume and initial["metadata"]["cache_fingerprint"] != cache.manifest["fingerprint"]:
        raise ValueError("V14 checkpoint/cache fingerprint differs")
    ...
    # The serializer also supports small CPU fixtures. A real 2048-wide run
    # must nevertheless prove the intended 128-projection rank-eight topology.
    if config.feature_dim == 2048 and (
            parent_info["config"]["expert"] != {"rank": 8, "alpha": 16.0}
            or parent_info["config"]["expert_targets"] != EXPECTED_REAL_EXPERT_TARGETS):
        raise ValueError("Production V14 requires all 128 rank8/alpha16 expert adapters")
    ...
    if args.preflight_only:
        print(json.dumps({"driver_variant": DRIVER, "start_step": start, "planned_updates": args.max_steps,
            ...
            "initial_parent": parent_record, "initialization_reference": reference,
            "trainable_scope": OBJECTIVE["trainable_scope"], "expert_config": parent_info["config"]["expert"],
            "expert_target_count": len(parent_info["config"]["expert_targets"]),
            ...
            with redirect_stdout(_Tee(sys.stdout, stream)), redirect_stderr(_Tee(sys.stderr, stream)):
                try:
                    ...
                    _atomic_json(output / "failure.json", {"error": type(exc).__name__, "message": str(exc),
                        "note": "Inspect failure; previously complete checkpoints and all original inputs are retained"})
                    raise
...
def run(args, cache, base, source, initial, parent_record, parent_info, episodes, plan, plan_sha, sources, output, visual_cfg, reference, sidecars, base_record):
    ...
    optimizer = torch.optim.AdamW(optimizer_groups(args, visual, head))
    optimizer_parameter_names(optimizer, visual, head)
    start = initial["step"] if args.resume else 0
    ...
    if args.resume:
        restored = load_checkpoint(source, visual, head, optimizer)
        ...
        _seed(args.seed + 102)
    config = {"trainer_variant": DRIVER, "driver_variant": DRIVER, "stage": 1, "mode": "visual_expert",
        "architecture": "visual_demo_tail_v13", "expert": copy.deepcopy(cfg["expert"]),
        "expert_targets": list(cfg["expert_targets"]),
    ...
    metadata = {"driver_variant": DRIVER, "base_model": checkpoint_identity(base), "initial_parent": parent_record,
        ...
        "note": "Visual encoding/retrieval and original-topology expert LoRA train jointly. Original base/archive files remain required and unchanged. READ-off retains the NEW expert. Offline errors are not robot success."}
    ...
    print(f"[visual-expert-v14] visual={sum(p.numel() for p in visual.parameters())}, "
          f"expert_LoRA={sum(p.numel() for p in expert_parameters(head))} parameters train; "
          f"original base/archive/CVOM frozen; start={start}/{args.max_steps}", flush=True)
    ...
    def evaluate(step):
        ...
        _atomic_json(output / f"validation-{step:06d}.json", {"step": step, "plan_sha256": plan_sha,
            ...
            "note": "Native Euler4; visual-off keeps this checkpoint\u0027s adapted AE and frozen archive, not the initial parent or original HAMLET; no task accuracy."})
        score = summaries["reader"]["generated_observed_prefix_mae"]
        print(f"[visual-expert-v14][val] step={step} prefix_MAE={score:.8f}; "
              f"adapted_off_MAE={summaries['visual-off']['generated_observed_prefix_mae']:.8f}; "
    ...
    def save(step, best=False):
        ...
        path = save_checkpoint(output, step, visual, head, optimizer, config, metadata, best=best,
    ...
    for step in range(start + 1, end + 1):
        ...
        for group in optimizer.param_groups:
            group["lr"] = (args.visual_learning_rate if group["kind"] == "visual" else args.expert_learning_rate) * factor
        ...
        norm = torch.nn.utils.clip_grad_norm_(joint_parameters(visual, head), args.max_grad_norm, error_if_nonfinite=True)
        optimizer.step()
        assert_finite_optimizer(optimizer)
        if not all(bool(torch.isfinite(p).all()) for p in joint_parameters(visual, head)):
            raise FloatingPointError("Nonfinite visual/expert parameters after optimizer update")
        assert_scope(visual, parent, head, cvom, guard)
        ...
        row.update(grad_norm=float(norm), learning_rate=optimizer.param_groups[0]["lr"],
            expert_learning_rate=next(g["lr"] for g in optimizer.param_groups if g["kind"] == "expert"),
        ...
        if step == 1 or step % args.log_steps == 0 or step == end:
            ...
            values.update({key: row[key] for key in ("processed_queries", "elapsed_seconds", "learning_rate", "expert_learning_rate")})
            logger.log(step, "train", values)
            interval = []
            print(f"[visual-expert-v14][train] step={step}/{args.max_steps} flow={row['loss']:.8f} grad={float(norm):.6f}", flush=True)
        best = False
    ...
    print(f"[visual-expert-v14] {state['status']}; {output / 'last_checkpoint.json'}", flush=True)
    return 0
