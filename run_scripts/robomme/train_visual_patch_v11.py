#!/usr/bin/env python3
"""Bounded V11 visual-only pilot over an immutable archive1250 + Action Expert.
...
success, or convergence. V11 bundles require their hash-bound external parent.
"""
...
from run_scripts.robomme.checkpoint_visual_patch_v11 import checkpoint_info, load_checkpoint, parent_reference, save_checkpoint
...
def source_identity():
    names = ("train_visual_patch_v11.py", "checkpoint_visual_patch_v11.py", "visual_patch_memory_v11.py",
             "replay_visual_patch_v11.py", "train_archive_deployment_v9.py", "verify_projector_v10.py",
...
def parser():
    ...
    source.add_argument("--resume", help="Exact V11 resume into a NEW directory; external parent required")
...
def parse_args(argv=None):
    ...
    if args.resume:
        ...
        if info["config"].get("trainer_variant") != DRIVER:
            raise ValueError("Exact resume requires a visual_patch_v11 checkpoint")
    ...
    args.driver_variant, args.stage, args.mode = DRIVER, 1, "visual_patch"
    return args
...
def visual_config(parent_info):
    ...
    return VisualPatchConfig(feature_dim=memory["feature_dim"], num_short_tokens=memory["num_short_tokens"],
...
def conditioned_episode(args, visual, episode, query, fused_short, *, enabled=True):
    ...
    features, bank = replay_visual_patch(visual, observations, query, camera_order=CAMERA_ORDER,
...
def validate(args, visual, parent, head, episodes, plan):
    ...
    return {role: _mean([{k: v for k, v in row.items() if k not in excluded} for row in rows if row["role"] == role])
            for role in ("reader", "visual-off")}, rows
...
def resume_options(args, info, sources):
    if info["config"].get("objective") != OBJECTIVE or info["metadata"].get("selection_metric") != SELECTION:
        raise ValueError("Exact V11 objective/selection semantics changed")
...
def validate_resume_state(info, plan, plan_sha):
    ...
    for key in ("best_generated_prefix_mse", "elapsed_seconds"):
        value = state.get(key)
...
def main(argv=None):
    ...
    if args.resume and initial["metadata"]["cache_fingerprint"] != cache.manifest["fingerprint"]:
        raise ValueError("V11 checkpoint/cache fingerprint differs")
    ...
    if args.preflight_only:
        print(json.dumps({"driver_variant": DRIVER, "start_step": start, "planned_updates": args.max_steps,
            ...
            "plan_sha256": plan_sha, "visual_config": asdict(config), "frozen_parent": parent_record,
            ...
            with redirect_stdout(_Tee(sys.stdout, stream)), redirect_stderr(_Tee(sys.stderr, stream)):
                try:
                    return run(args, cache, base, source, initial, parent_record, parent_info, episodes,
                               plan, plan_sha, sources, output, config)
...
def run(args, cache, base, source, initial, parent_record, parent_info, episodes, plan, plan_sha, sources, output, visual_cfg):
    _seed(args.seed)
    ...
    with isolated_seed(args.seed + 100, args.device):
        ...
        visual = VisualPatchMemoryV11(visual_cfg).to(args.device)
    ...
    state = copy.deepcopy(initial["metadata"]["train_state"]) if args.resume else {
        "optimizer_updates": 0, "window_cursor": 0, "processed_queries": 0, "best_generated_prefix_mse": None,
    ...
    if args.resume:
        ...
        _seed(args.seed + 102)
    config = {"trainer_variant": DRIVER, "driver_variant": DRIVER, "stage": 1, "mode": "visual_patch",
        "visual": asdict(visual_cfg), "camera_order": list(CAMERA_ORDER), "train": vars(args).copy(),
    ...
    metadata = {"driver_variant": DRIVER, "base_model": checkpoint_identity(base), "frozen_parent": parent_record,
        ...
        "selection_metric": SELECTION, "self_contained": False,
    ...
    print(f"[visual-v11] ONLY {sum(p.numel() for p in visual.parameters())} visual parameters train; "
    ...
    def evaluate(step):
        ...
        print(f"[visual-v11][val] step={step} prefix={score:.8f}; parent={summaries['visual-off']['generated_observed_prefix_mse']:.8f}", flush=True)
        return score
    ...
    for step in range(start + 1, end + 1):
        ...
        if step == 1 or step % args.log_steps == 0 or step == end:
            ...
            print(f"[visual-v11][train] step={step}/{args.max_steps} flow={row['loss']:.8f} grad={float(norm):.6f}", flush=True)
        best = False
        ...
        if step % args.plot_steps == 0 or step == end:
            logger.plot()
    _atomic_json(output / "status.json", state)
    print(f"[visual-v11] {state['status']}; {output / 'last_checkpoint.json'}", flush=True)
    return 0
