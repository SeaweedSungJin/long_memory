#!/usr/bin/env python3
"""Paired RoboMME evaluation with the saved v4 Action Expert adapter.

Standalone versioned driver. Original baseline is never silently adapted.
reader uses Stage 1, memory/fifo share Stage 2 weights, and expert-only disables
memory on the SAME Stage 1 checkpoint (inference ablation, not trained control).
Existing evaluation folders resume only under identical provenance.
"""
...
from gr00t.eval.sim.robomme.compare_long_memory_v4_results import write_v4_report as write_report
...
def build_parser():
    ...
    parser.add_argument("--reader-checkpoint", type=Path, help="Adapted v4 Stage 1 bundle, NOT v3")
    parser.add_argument("--memory-checkpoint", type=Path, help="Adapted v4 Stage 2 bundle, NOT v3")
    parser.add_argument("--models", nargs="+", choices=("baseline", "reader", "memory", "fifo", "expert-only"),
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/eval/robomme/action_expert_v4_smoke"))
...
def validate_options(args):
    ...
    if any(name in args.models for name in ("reader", "expert-only")) and args.reader_checkpoint is None:
        raise ValueError("reader/expert-only requires --reader-checkpoint from v4 Stage 1")
    if any(name in args.models for name in ("memory", "fifo")) and args.memory_checkpoint is None:
        raise ValueError("memory/fifo requires --memory-checkpoint from v4 Stage 2")
...
def build_identity(args):
    """No expensive model construction; check both small memory and AE files."""
    from gr00t.long_memory.checkpoint_v4 import v4_checkpoint_info
    ...
    if (config.get("hamlet_mode") != "finetune" or config.get("mem_cond_type", "cross_attn") != "cross_attn"
        ...
        raise ValueError("V4 evaluation requires trained HAMLET moment-token/cross-attention weights")
    ...
    for name in args.models:
        entry = {"base_model": base_identity, "memory_checkpoint": None, "write_policy": "none",
                 "write_policy_override": "checkpoint", "expert_only": False, "reader_mode": "none",
        ...
        if name != "baseline":
            ...
            if checkpoint not in checked:
                info = v4_checkpoint_info(base, checkpoint, expected_stage=stage)
                checked[checkpoint] = (info, {filename: file_hash(checkpoint / filename) for filename in
                                             ("checkpoint.json", "model.safetensors", "expert.safetensors")})
            info, hashes = checked[checkpoint]
            if info["config"]["stage"] != stage:
                raise ValueError(f"Wrong checkpoint stage for model role {name}")
            cfg = info["config"]
            mode = cfg.get("train", {}).get("reader_mode", cfg.get("reader_mode"))
            # Different adapters/readers cannot form a writer-isolation cohort.
            cohorts.append((info["metadata"]["cache_fingerprint"], cfg["memory"],
                            cfg["expert"], cfg["expert_targets"], mode))
            disabled = name == "expert-only" or mode == "none"
            if name == "expert-only":
                description = "Same adapted Stage 1 expert; memory disabled at inference, NOT a trained no-memory control"
            elif mode == "none":
                description = "Action Expert trained without long memory (reader_mode=none control)"
            elif name == "reader":
                description = "Stage 1 jointly adapted Action Expert + long memory, FIFO storage"
            elif name == "fifo":
                description = "Same Stage 2 expert + reader as memory role, FIFO storage ablation"
            else:
                description = "Stage 2 frozen adapted Action Expert + reader, learned storage policy"
            entry.update(memory_checkpoint=str(checkpoint), checkpoint_sha256=hashes["checkpoint.json"],
                         weights_sha256=hashes["model.safetensors"], expert_weights_sha256=hashes["expert.safetensors"],
                         stage=stage, step=info["step"], memory_config=cfg["memory"], expert_config=cfg["expert"],
                         expert_targets=cfg["expert_targets"], reader_mode=mode,
                         training_metadata=info["metadata"], trainer_variant=VARIANT,
                         write_policy="none" if disabled else "hard" if name == "memory" else "all",
                         write_policy_override="all" if name == "fifo" else "checkpoint",
                         expert_only=name == "expert-only", description=description)
        models[name] = entry
    if cohorts and any(value != cohorts[0] for value in cohorts[1:]):
        raise ValueError("Compared v4 models must share training cache, memory/adapter architecture and reader mode")
    ...
    sources.update(Path(name) for name in ("run_scripts/robomme/serve_long_memory_v4.py",
                   "run_scripts/robomme/eval_long_memory_v4.py", "run_scripts/robomme/eval_long_memory_comparison.py"))
    ...
    print("[preflight] hashing original checkpoint and adapted expert/memory for evaluation provenance ...", flush=True)
...
def server_command(args, model, port):
    command = [str(python_path(args.server_python)), "-u", str(REPO_ROOT / "run_scripts/robomme/serve_long_memory_v4.py"),
...
def run_evaluation(args, identity, env):
    ...
    try:
        for name, model in identity["models"].items():
            ...
            for task in args.tasks:
                ...
                if len(rows) != args.n_episodes:
                    ...
                    print(f"[v4-eval] resume: {name}/{task} already complete", flush=True)
            if not pending:
                continue
            folder = output / name
            ...
            with (folder / "server.log").open("a", encoding="utf-8") as log:
                try:
                    print(f"[v4-eval] loading {name}; {folder / 'server.log'}", flush=True)
                    ...
                    for task in pending:
                        ...
                        print(f"[v4-eval] {name}/{task}; {task_dir / 'rollout.log'}", flush=True)
                        try:
                            ...
                            print(f"[v4-eval] ERROR {name}/{task}: {exc}", file=sys.stderr, flush=True)
                        write_report(output, bootstrap_samples=500)
                except (RuntimeError, TimeoutError) as exc:
                    failures.append({"model": name, "error": str(exc)})
                    print(f"[v4-eval] ERROR {name}: {exc}; see server.log", file=sys.stderr, flush=True)
                finally:
                    stop_process(server)
        ...
        try:
            ...
            print(f"[v4-eval] Summary: {output / 'comparison_summary.txt'}", flush=True)
        finally:
            lock.close()
...
def main(argv=None):
    ...
    if args.report_only:
        ...
        if manifest.get("trainer_variant") != VARIANT:
            raise ValueError("Not a v4 evaluation; use the matching older report tool")
    ...
    print(f"[preflight] compatible v4 models={list(identity['models'])}; tasks={args.tasks}; n={args.n_episodes}", flush=True)
    if args.preflight_only:
...
if __name__ == "__main__":
    try:
        ...
        print("\n[v4-eval] Interrupted. Owned processes stopped; completed episodes remain resumable.", file=sys.stderr)
