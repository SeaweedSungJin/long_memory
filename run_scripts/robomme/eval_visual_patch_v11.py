#!/usr/bin/env python3
"""Paired V11 visual READ evaluation with a frozen archive/Action Expert parent.

Only visual-on versus visual-off isolates the new spatial-memory path. Original
HAMLET is a separate baseline. External baseline results keep their original
...
V11 model identities and actual per-call visual diagnostics are checked here.
"""
...
SERVER = Path("run_scripts/robomme/serve_visual_patch_v11.py")
...
EVALUATOR = Path("run_scripts/robomme/eval_visual_patch_v11.py")
...
def build_parser():
    ...
    parser.add_argument("--visual-checkpoint", type=Path, help="One trained visual_patch_v11 checkpoint for BOTH visual roles")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/eval/robomme/visual_patch_v11_val_n10_seed6"))
...
def validate_options(args):
    ...
    if any(role != "baseline" for role in args.models) and args.visual_checkpoint is None:
...
def validate_manifest_contract(identity):
    ...
    if identity.get("format_version") != 1 or identity.get("trainer_variant") != VARIANT:
        raise ValueError("Expected visual_patch_v11 evaluation")
    ...
    for name, model in models.items():
        if model.get("memory_off") is not False or model.get("archive_read_off") is not False:
            raise ValueError("V11 never disables the frozen parent's short archive")
        ...
        if name == "baseline":
            ...
            continue
        if (model.get("trainer_variant") != VARIANT or model.get("mode") != "visual_patch"
            ...
            raise ValueError("Requires a trained V11 checkpoint with unchanged APPEND")
        ...
        if (not isinstance(files, dict) or set(files) != set(aliases.values()) or any(not _hash(v) for v in files.values())
            ...
            raise ValueError("V11 visual checkpoint hashes missing or inconsistent")
    ...
    if {"visual", "visual-off"} <= set(models):
        ...
        if a != b:
            raise ValueError("Visual on/off must use the SAME frozen parent and visual checkpoint")
...
def build_identity(args):
    ...
    from run_scripts.robomme.checkpoint_visual_patch_v11 import checkpoint_info
    ...
    if any(name != "baseline" for name in args.models):
        checkpoint = resolve_repo_path(args.visual_checkpoint)
        info = checkpoint_info(base, checkpoint, expected_stage=1)
        cfg, meta = info["config"], info["metadata"]
        if info["step"] <= 0:
            raise ValueError("Actual model evaluation requires trained step>0, not initialization")
        ...
        common = {
            ...
            "stage": 1, "step": info["step"], "mode": "visual_patch",
    ...
    descriptions = {
        ...
        "visual": "Frozen archive1250/AE plus learned visual encoding and full-history READ",
        "visual-off": "SAME frozen archive1250/AE and visual checkpoint; only visual READ bypassed, APPEND continues",
    }
    models = {}
    for name in args.models:
        model = copy.deepcopy(common) if name != "baseline" else {
    ...
    print("[preflight] hashing original HAMLET, frozen parent, and V11 visual weights ...", flush=True)
    identity = {
        ...
        "control_contract": "identical frozen archive1250/AE/visual weights, only visual READ bypass; APPEND and noise unchanged",
...
def completed_visual_diagnostics(root, role, tasks, expected, model):
    ...
    totals = {k: 0 for k in ("completed_sessions", "missing_sessions", "calls", "missing_calls",
               ...
               "append_updates", "demo_updates", "ignored_torn_final_lines")}
    ...
    for task in tasks:
        ...
        for episode, row in rows.items():
            ...
            for index, record in enumerate(records, 1):
                ...
                enabled = role == "visual" and not passive and index > 1
...
def build_control_report(run_dir, *, bootstrap_samples=5000):
    ...
    lines = [text.rstrip(), "", "V11: ONLY new visual READ differs; archive1250 and complete AE/LoRA stay frozen."]
    if {"visual", "visual-off"} <= set(models):
        for task in settings["tasks"]:
            contexts = [validate_result_identity(root, role, task, manifest) for role in ("visual-off", "visual")
            ...
            if len(contexts) == 2 and any(contexts[0].get(k) != contexts[1].get(k) for k in
                ...
                raise ValueError("Visual on/off scenario or short-memory context differs")
        comparison = _contrast(root, "visual-off", "visual", settings["tasks"], settings["n_episodes"], bootstrap_samples)
        result["additional_comparisons"]["visual-off_to_visual"] = comparison
        lines += ["visual-off -> visual: learned visual encoding/READ contribution; original short archive remains ON",
    ...
    lines += ["Visual-off retains frozen short-archive READ and exactly the same frozen AE; it is NOT all-memory-off.",
              "Both banks still APPEND original observations in visual-off. Different actions may change rollout/write counts.",
              "Visual storage representation and retrieval are learned; admission is rule-based, not CVOM/learned merge.",
              "All-history patch storage is not a bounded-capacity or compute-matched memory control.",
    ...
    try:
        for name, model in identity["models"].items():
            if name == "baseline" and "baseline_reference" in identity:
                ...
                print(f"[visual-v11-eval] baseline REUSED: {reference['completed_episodes']} completed; "
            ...
            for task in args.tasks:
                ...
                if len(rows) != args.n_episodes:
                    ...
                    print(f"[visual-v11-eval] resume: {name}/{task} complete", flush=True)
            if not pending:
                continue
            folder = output / name
            ...
            with (folder / "server.log").open("a", encoding="utf-8") as log:
                try:
                    ...
                    for task in pending:
                        ...
                        print(f"[visual-v11-eval] {name}/{task}; {task_dir / 'rollout.log'}", flush=True)
                        try:
                            ...
                            print(f"[visual-v11-eval] ERROR {name}/{task}: {exc}", file=sys.stderr, flush=True)
                        write_report(output, bootstrap_samples=500)
                except (RuntimeError, TimeoutError) as exc:
                    failures.append({"model": name, "error": str(exc)})
                    print(f"[visual-v11-eval] ERROR {name}: {exc}; see server.log", file=sys.stderr, flush=True)
                finally:
                    stop_process(server)
        ...
        try:
            if fatal is None:
                try:
                    ...
                    print(f"[visual-v11-eval] Summary: {output / 'comparison_summary.txt'}", flush=True)
                except BaseException as exc:
                    fatal = {"error_type": type(exc).__name__, "error": str(exc)}
...
def main(argv=None):
    ...
    with (output.parent / ("." + output.name + ".visual-patch-v11-launch.lock")).open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
...
if __name__ == "__main__":
    try:
        ...
        print("\n[visual-v11-eval] Interrupted; owned processes stopped, completed episodes resumable.", file=sys.stderr)
        raise SystemExit(130)
    except (ValueError, RuntimeError, FileNotFoundError, subprocess.TimeoutExpired) as exc:
        print(f"[visual-v11-eval] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
