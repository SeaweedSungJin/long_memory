#!/usr/bin/env python3
"""Paired V13 visual READ evaluation with a frozen archive/Action Expert parent.

Tail/canonical are independently trained, matched-budget differential readers.
Tail/visual-off share one checkpoint and ingest the same uniform demo tail.
Original HAMLET is a separate baseline. Primary checkpoints are fixed final-step;
MAE-best selection remains a training diagnostic, not a rollout selection rule.
...
V13 model identities and actual per-call visual diagnostics are checked here.
"""
...
SERVER = Path("run_scripts/robomme/serve_demo_tail_v13.py")
...
EVALUATOR = Path("run_scripts/robomme/eval_demo_tail_v13.py")
...
ROLES = ("baseline", "tail", "canonical", "visual-off")
DEPENDENCIES = tuple(Path("run_scripts/robomme") / name for name in (
    "policy_demo_tail_v13.py", "checkpoint_demo_tail_v13.py", "visual_differential_memory_v12.py",
...
def build_parser():
    ...
    parser.add_argument("--tail-checkpoint", type=Path, help="Actual include_tail=True final bundle; also unchanged for visual-off")
    parser.add_argument("--canonical-checkpoint", type=Path, help="Independently trained include_tail=False final bundle")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/eval/robomme/demo_tail_v13_val_n10_seed6"))
...
def validate_options(args):
    ...
    if set(args.models) & {"tail", "visual-off"} and args.tail_checkpoint is None:
        raise ValueError("tail/visual-off requires --tail-checkpoint")
    if "canonical" in args.models and args.canonical_checkpoint is None:
        raise ValueError("canonical requires --canonical-checkpoint")
    ...
    if args.n_action_steps != 16:
        raise ValueError("V13 preserves the exact canonical stride16 protocol")
...
def validate_manifest_contract(identity):
    ...
    if identity.get("format_version") != 1 or identity.get("trainer_variant") != VARIANT:
        raise ValueError("Expected demo_tail_v13 evaluation")
    ...
    for name, model in models.items():
        if model.get("memory_off") is not False or model.get("archive_read_off") is not False:
            raise ValueError("V13 never disables the frozen parent's short archive")
        ...
        if name == "baseline":
            ...
            continue
        expected_mode, expected_tail = "differential", name != "canonical"
        if (model.get("trainer_variant") != VARIANT or model.get("mode") != "visual_demo_tail"
            ...
            raise ValueError("Requires V13 checkpoint with unchanged APPEND; step0 needs explicit --allow-initialization-checkpoints")
        ...
        if (not isinstance(files, dict) or set(files) != set(aliases.values()) or any(not _hash(v) for v in files.values())
            ...
            raise ValueError("V13 visual checkpoint hashes missing or inconsistent")
        ...
        if (type(horizon) is not int or horizon < 1 or model["step"] > horizon
                or (model["step"] != horizon and not (allow_initialization and model["step"] == 0))):
            raise ValueError("V13 primary rollout requires the fixed final step; only explicit step0 initialization is exempt")
        ...
        if objective.get("rollout_selection") != "fixed_final_step" or objective.get("mae_best_is_diagnostic") is not True:
            raise ValueError("V13 rollout selection must be predeclared fixed final step, not best MAE")
        ...
        if (not isinstance(reference, dict) or not _hash(reference.get("initial_visual_sha256"))
            ...
            raise ValueError("V13 initial weights/full base/sidecar provenance is incomplete")
    ...
    if {"tail", "visual-off"} <= set(models):
        ignored = {"description", "visual_read_off"}
        a, b = ({k: v for k, v in models[n].items() if k not in ignored} for n in ("tail", "visual-off"))
        if a != b:
            raise ValueError("Tail/visual-off must use the SAME frozen parent and visual checkpoint")
        ...
        for model in candidates[1:]:
            for key in ("frozen_parent", "visual_config", "camera_order", "training_contract"):
                if model[key] != candidates[0][key]:
                    raise ValueError(f"V13 trained-arm comparison is unmatched: {key}")
...
def build_identity(args):
    ...
    from run_scripts.robomme.checkpoint_demo_tail_v13 import checkpoint_info
    ...
    bundles, inputs = {}, [base]
    required = {}
    if set(args.models) & {"tail", "visual-off"}:
        required["tail"] = args.tail_checkpoint
    if "canonical" in args.models:
        required["canonical"] = args.canonical_checkpoint
    for arm, path in required.items():
        ...
        bundles[arm] = {
            ...
            "stage": 1, "step": info["step"], "mode": "visual_demo_tail", "read_mode": "differential",
        ...
        inputs += [checkpoint, meta["frozen_parent"]["path"], meta["sidecar"]["path"]]
    ...
    descriptions = {
        ...
        "tail": "Frozen archive1250/AE plus trained differential reader with uniform missing demo-tail RGB evidence",
        "canonical": "Independently trained differential reader using canonical sparse evidence; SAME query/init/budget",
        "visual-off": "SAME tail bundle and tail ingestion, visual READ bypassed; original archive READ/APPEND unchanged",
    }
    models = {}
    for name in args.models:
        model = copy.deepcopy(bundles["canonical" if name == "canonical" else "tail"]) if name != "baseline" else {
    ...
    print("[preflight] hashing original HAMLET, frozen parent, and V13 visual weights ...", flush=True)
    identity = {
        ...
        "control_contract": "same frozen archive1250/AE; matched final-step tail/canonical differential readers; visual-off retains tail ingest and bypasses visual READ only",
...
def validate_task_result(root, role, task, manifest):
    ...
    if role != "baseline" and context is not None:
        ...
        if context.get("client_variant") != "demo_tail_v13" or context.get("demo_tail_ingest") is not model["include_tail"]:
            raise ValueError("V13 actual rollout client/tail arm differs from evaluation manifest")
    return context
...
def verify_runtime_inputs(identity):
    ...
    for path, digest in files.items():
        if file_hash(path) != digest:
            raise ValueError(f"V13 inference source/base/parent/bundle changed: {path}")
    return True
        ...
        for episode, row in rows.items():
            ...
            for index, record in enumerate(records, 1):
                ...
                for key, value in {
                    ...
                    if type(visual[key]) is not type(value) or visual[key] != value:
                        raise ValueError(f"V13 runtime read-mode/branch differs: {key}")
            ...
            with path.open(encoding="utf-8") as stream:
                for line in stream:
                    try:
                        ...
                        if not line.endswith("\n"):
                            continue
                        raise ValueError("Malformed complete V13 tail diagnostic")
                    sid = record.get("session_id")
        ...
        for episode, row in rows.items():
            ...
            for record in records:
                if record.get("session_id") != sid or record.get("episode_idx") != episode or record.get("episode_seed") != row["episode_seed"]:
                    raise ValueError("V13 tail RPC/call belongs to a different completed episode/session")
            decisions = [(position, record) for position, record in calls if record.get("passive") is False]
            if not decisions:
                raise ValueError("Completed V13 episode has no actual execution decision")
            first_position, first = decisions[0]
            n_demo = first.get("frame_index")
            if type(n_demo) is not int or n_demo < 0:
                raise ValueError("First V13 execution frame must be n_demo")
            ...
            if ([record.get("frame_index") for _, record in passive] != canonical
                ...
                raise ValueError("V13 canonical demo priming changed")
            ...
            if not include_tail:
                ...
                if len(events) != 1:
                    raise ValueError("Expected exactly one V13 ingest/skip event per completed session")
                ...
                if tail:
                    ...
                    if (event["kind"] != "demo_tail_ingest" or event.get("frames") != tail
                        ...
                        raise ValueError("V13 actual image-ingest RPC response/causal frames differ")
                    totals["ingest_rpc"] += 1
                    ...
                    if (event["kind"] != "demo_tail_skipped" or type(event.get("n_demo")) is not int
                        ...
                        raise ValueError("Invalid V13 no-demo/no-missing-frame skip evidence")
            ...
            for index, (_, call) in enumerate(calls):
                ...
                for key, value in wanted.items():
                    if key in diagnostic and (type(diagnostic[key]) is not type(value) or diagnostic[key] != value):
                        raise ValueError(f"V13 demo-tail diagnostic contradicts actual RPC/canonical bank: {key}")
...
def build_control_report(run_dir, *, bootstrap_samples=5000):
    ...
    lines = [text.rstrip(), "", "V13: tail/canonical differential readers; archive1250 and complete AE/LoRA stay frozen.",
             "Primary comparison uses each arm's predeclared final step; MAE-best is diagnostic only."]
    contrasts = (("visual-off", "tail", "same tail checkpoint and ingestion, visual READ contribution"),
                 ("canonical", "tail", "independently trained matched init/queries/budget, uniform demo-tail evidence"),
                 ("visual-off", "canonical", "trained canonical sparse reader vs frozen parent"))
    for control, candidate, description in contrasts:
        ...
        for task in settings["tasks"]:
            ...
            if len(contexts) == 2 and any(contexts[0].get(k) != contexts[1].get(k) for k in
                ...
                raise ValueError("V13 paired scenario or short-memory context differs")
    ...
    lines += ["Visual-off retains frozen short-archive READ and exactly the same frozen AE; it is NOT all-memory-off.",
              "All roles preserve canonical APPEND counts; tail/visual-off additionally ingest the same separate image-only bank.",
              "Both trained readers use past-minus-current READ; only available demo-tail evidence differs. Both bypass empty history.",
              "Visual storage representation and retrieval are learned; admission is rule-based, not CVOM/learned merge.",
              "All-history storage is unbounded; tail uses more encoding/storage/attention tokens, so this is not equal compute.",
    ...
    try:
        for name, model in identity["models"].items():
            if name == "baseline" and "baseline_reference" in identity:
                ...
                print(f"[demo-tail-v13-eval] baseline REUSED: {reference['completed_episodes']} completed; "
            ...
            for task in args.tasks:
                ...
                if len(rows) != args.n_episodes:
                    ...
                    print(f"[demo-tail-v13-eval] resume: {name}/{task} complete", flush=True)
            if not pending:
                continue
            folder = output / name
            ...
            with (folder / "server.log").open("a", encoding="utf-8") as log:
                try:
                    ...
                    for task in pending:
                        ...
                        print(f"[demo-tail-v13-eval] {name}/{task}; {task_dir / 'rollout.log'}", flush=True)
                        try:
                            ...
                            print(f"[demo-tail-v13-eval] ERROR {name}/{task}: {exc}", file=sys.stderr, flush=True)
                        write_report(output, bootstrap_samples=500)
                except (RuntimeError, TimeoutError) as exc:
                    failures.append({"model": name, "error": str(exc)})
                    print(f"[demo-tail-v13-eval] ERROR {name}: {exc}; see server.log", file=sys.stderr, flush=True)
                finally:
                    stop_process(server)
        ...
        try:
            ...
            if fatal is None:
                try:
                    ...
                    print(f"[demo-tail-v13-eval] Summary: {output / 'comparison_summary.txt'}", flush=True)
                except BaseException as exc:
                    fatal = {"error_type": type(exc).__name__, "error": str(exc)}
...
def main(argv=None):
    ...
    with (output.parent / ("." + output.name + ".demo-tail-v13-launch.lock")).open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
...
if __name__ == "__main__":
    try:
        ...
        print("\n[demo-tail-v13-eval] Interrupted; owned processes stopped, completed episodes resumable.", file=sys.stderr)
        raise SystemExit(130)
    except (ValueError, RuntimeError, FileNotFoundError, subprocess.TimeoutExpired) as exc:
        print(f"[demo-tail-v13-eval] ERROR: {exc}", file=sys.stderr)
        raise SystemExit(1)
