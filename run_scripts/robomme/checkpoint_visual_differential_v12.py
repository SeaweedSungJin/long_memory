"""Immutable visual-only V12 bundles with an explicit EXTERNAL frozen parent.
...
def _finite_tree(value):
    if torch.is_tensor(value):
        if not bool(torch.isfinite(value).all()):
            raise ValueError("V12 nonfinite training state")
    elif isinstance(value, dict):
        for key, item in value.items():
            if not isinstance(key, (str, int)):
                raise ValueError("V12 unsupported training-state key")
            _finite_tree(item)
        ...
        raise ValueError("V12 unsupported training-state value")


def _config(config):
    if not isinstance(config, dict) or config.get("trainer_variant") != VARIANT:
        raise ValueError("Expected visual_differential_v12; other checkpoint architectures are incompatible")
    if config.get("driver_variant", VARIANT) != VARIANT:
        raise ValueError("V12 driver identity differs")
    if type(config.get("stage")) is not int or config["stage"] != 1 or config.get("mode") != "visual_differential":
        raise ValueError("V12 supports Stage 1 visual_differential mode only")
    read_mode = config.get("read_mode")
    if (read_mode not in ("differential", "current_only")
            or config.get("train", {}).get("read_mode") != read_mode):
        raise ValueError("V12 exact differential/current_only training-arm identity required")
    if config.get("camera_order") != list(CAMERA_ORDER):
        raise ValueError("V12 exact audited camera order required")
    spec = config.get("visual")
    if not isinstance(spec, dict) or set(spec) != {f.name for f in fields(VisualDifferentialConfig)}:
        raise ValueError("V12 exact visual-memory specification required")
...
def parent_reference(base_model, parent_path):
    ...
    if info["config"]["mode"] != "archive" or info["step"] != 1250:
        raise ValueError("V12 requires the frozen V7 Stage-1 archive checkpoint at step 1250")
...
def _metadata(base_model, config, metadata):
    ...
    if not isinstance(metadata, dict) or metadata.get("base_model") != checkpoint_identity(base_model):
        raise ValueError("V12 original base identity differs")
    if metadata.get("read_mode") != config["read_mode"]:
        raise ValueError("V12 metadata read_mode differs from checkpoint configuration")
    ...
    if (not isinstance(parent, dict) or set(parent) != {"path", "step", "files_sha256"}
        ...
        raise ValueError("V12 explicit external frozen-parent reference required")
    if parent_reference(base_model, parent["path"]) != parent:
        raise ValueError("V12 frozen parent changed")
    ...
    for key in ("feature_dim", "num_short_tokens", "time_scale"):
        if getattr(cfg, key) != parent_info["config"]["memory"][key]:
            raise ValueError(f"V12 visual {key} differs from frozen parent")
    if not isinstance(metadata.get("cache_fingerprint"), str) or not metadata["cache_fingerprint"]:
        raise ValueError("V12 cache fingerprint required")
    ...
    if not isinstance(sources, dict) or not sources or any(not isinstance(k, str) or not _sha(v) for k, v in sources.items()):
        raise ValueError("V12 explicit source hashes required")
    if not isinstance(metadata.get("runtime"), dict) or not metadata["runtime"]:
        raise ValueError("V12 runtime provenance required")
    ...
    with torch.random.fork_rng(devices=[]), torch.device("meta"):
        visual = VisualDifferentialMemoryV12(cfg, read_mode=config["read_mode"])
...
def _state(state, shapes, step):
    if not isinstance(state, dict) or set(state) != set(shapes):
        raise ValueError("V12 visual tensor names differ")
    for name, value in state.items():
        if not torch.is_tensor(value) or tuple(value.shape) != shapes[name] or value.dtype != torch.float32:
            raise ValueError(f"V12 visual shape/dtype differs: {name}")
        if not bool(torch.isfinite(value).all()):
            raise ValueError(f"V12 nonfinite visual tensor: {name}")
    if step == 0 and bool(state["output_projection.weight"].count_nonzero()):
        raise ValueError("V12 step-zero visual output projection must be zero")


def _installed(visual, config):
    if not isinstance(visual, VisualDifferentialMemoryV12) or asdict(visual.config) != asdict(_config(config)):
        raise ValueError("V12 installed visual semantic config differs")
    if visual.read_mode != config["read_mode"]:
        raise ValueError("V12 installed read_mode differs; never coerce a training arm")
    if any(p.dtype != torch.float32 for p in visual.parameters()):
        raise ValueError("V12 visual master parameters must be FP32")


def _optimizer_names(optimizer, visual):
    if not isinstance(optimizer, torch.optim.AdamW):
        raise ValueError("V12 supports AdamW exact resume only")
    allowed = {id(p): name for name, p in visual.named_parameters()}
    seen, result = set(), []
    for group in optimizer.param_groups:
        if group.get("kind") != "visual":
            raise ValueError("V12 optimizer groups must be visual-only")
        names = []
        for parameter in group["params"]:
            if id(parameter) not in allowed or id(parameter) in seen or not parameter.requires_grad:
                raise ValueError("V12 optimizer contains duplicate, frozen or non-visual parameters")
            seen.add(id(parameter))
            names.append(allowed[id(parameter)])
        if not names:
            raise ValueError("V12 empty optimizer group")
        result.append(names)
    if seen != set(allowed):
        raise ValueError("V12 optimizer must own every visual parameter exactly once")
    return result


def _optimizer(saved, names, shapes, step, current=None):
    if not isinstance(saved, dict) or set(saved) != {"state", "param_groups"}:
        raise ValueError("V12 invalid AdamW state")
    ...
    if not isinstance(groups, list) or not isinstance(states, dict) or not isinstance(names, list) or len(groups) != len(names):
        raise ValueError("V12 optimizer groups differ")
    ...
    for index, (group, keys) in enumerate(zip(groups, names)):
        ...
        for identifier, name in zip(group["params"], keys):
            if type(identifier) is not int or identifier in seen or name not in shapes or name in named:
                raise ValueError("V12 optimizer parameter ownership/order invalid")
            seen.add(identifier)
            named.add(name)
            ...
            if not isinstance(values, dict) or (values and set(values) != required):
                raise ValueError("V12 invalid AdamW moment fields")
            if not values:
                continue
            count = values["step"]
            if (not torch.is_tensor(count) or count.ndim != 0 or count.dtype not in (torch.float32, torch.float64)
                ...
                raise ValueError("V12 invalid optimizer-boundary step")
            for key in required - {"step"}:
                value = values[key]
                if not torch.is_tensor(value) or tuple(value.shape) != shapes[name] or value.dtype != torch.float32:
                    raise ValueError("V12 optimizer moment shape/dtype differs")
                if not bool(torch.isfinite(value).all()) or (key != "exp_avg" and bool((value < 0).any())):
                    raise ValueError("V12 invalid AdamW moment")
    if named != set(shapes) or set(states) - seen:
        raise ValueError("V12 optimizer must own all and only visual parameters")
...
def _training(training, step, shapes, *, optimizer=None, names=None, restore=False):
    if (not isinstance(training, dict) or set(training) != {"optimizer", "optimizer_param_names", "rng", "extra", "optimizer_boundary"}
        ...
        raise ValueError("V12 invalid optimizer-boundary training state")
    ...
    if (not isinstance(extra, dict) or extra.get("driver_variant") != VARIANT
        ...
        raise ValueError("V12 driver/cursor/plan identity differs")
    rng = training["rng"]
    if not isinstance(rng, dict) or set(rng) != {"python", "numpy", "torch", "cuda"}:
        raise ValueError("V12 invalid RNG state")
    ...
    if rng["cuda"] is not None and (not isinstance(rng["cuda"], (tuple, list)) or any(
        ...
        raise ValueError("V12 invalid CUDA RNG tensors")
    if training["optimizer"] is None:
        if restore or training["optimizer_param_names"] is not None:
            raise ValueError("V12 checkpoint has no optimizer state")
    else:
        ...
        if names is not None and names != training["optimizer_param_names"]:
            raise ValueError("V12 optimizer parameter ownership/order differs")
...
def _read(base_model, path, expected_stage=1):
    ...
    if type(info.get("format_version")) is not int or info["format_version"] != 1:
        raise ValueError("Unsupported V12 checkpoint format")
    ...
    if expected_stage is not None and (type(expected_stage) is not int or expected_stage != 1):
        raise ValueError("V12 expected stage must be 1")
    if info.get("self_contained") is not False:
        raise ValueError("V12 bundle must declare its external frozen-parent dependency")
    ...
    if meta.get("payload_sha256") != {"visual.safetensors": file_sha256(path / "visual.safetensors")}:
        raise ValueError("V12 visual payload changed")
    ...
    if meta.get("training_state_sha256") != file_sha256(path / "training_state.pt"):
        raise ValueError("V12 optimizer/RNG state changed")
...
def save_checkpoint(output_dir, step, visual, optimizer, config, metadata, best=False, training_state=None):
    ...
    if type(best) is not bool:
        raise ValueError("V12 best must be boolean")
    ...
    for protected in (Path(info["metadata"]["base_model"]["path"]).resolve(), Path(info["metadata"]["frozen_parent"]["path"]).resolve()):
        if root == protected or root.is_relative_to(protected):
            raise ValueError("V12 output cannot modify its frozen base/parent directory")
    destination = root / f"checkpoint-{step:06d}"
    if destination.exists():
        raise FileExistsError(f"V12 checkpoint already exists: {destination}")
