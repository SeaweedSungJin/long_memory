...
class VisualCheckpointTests(unittest.TestCase):
    def setUp(self):
        ...
        self.visual_config = VisualDifferentialConfig(feature_dim=8, hidden_dim=8, num_heads=2)
        self.visual = VisualDifferentialMemoryV12(self.visual_config)
        self.config = {"trainer_variant": checkpoint.VARIANT, "driver_variant": checkpoint.VARIANT,
                       "stage": 1, "mode": "visual_differential", "visual": asdict(self.visual_config),
                       "read_mode": "differential", "camera_order": list(CAMERA_ORDER),
                       "train": {"objective": "flow", "read_mode": "differential"}}
        self.metadata = {"base_model": checkpoint_identity(self.base), "cache_fingerprint": "toy-cache",
                         ...
                         "read_mode": "differential", "source_sha256": {"fixture.py": "a" * 64},
    ...
    def test_roundtrip_external_parent_immutable_and_readonly_rng(self):
        ...
        from run_scripts.robomme.checkpoint_visual_patch_v11 import checkpoint_info as old_info
        with self.assertRaisesRegex(ValueError, "visual_patch_v11"):
            old_info(self.base, path)

    def test_arm_semantics_strict_and_current_only_roundtrip(self):
        path = self.save()
        other = VisualDifferentialMemoryV12(self.visual_config, read_mode="current_only")
        ...
        self.assert_tree(before, other.state_dict())
        self.assert_tree(rng, _rng_state())
        for location in (self.config, self.config["train"], self.metadata):
            location["read_mode"] = "current_only"
        with self.assertRaisesRegex(ValueError, "read_mode"):
            self.save("wrong_arm")
        self.visual = other
        saved = self.save("current")
        checkpoint.load_checkpoint(saved, other)
        self.assertEqual(checkpoint.checkpoint_info(self.base, saved)["config"]["read_mode"], "current_only")
        for location in (self.config, self.config["train"], self.metadata):
            location["read_mode"] = "differential"
            with self.assertRaisesRegex(ValueError, "read_mode|identity"):
                self.save("mismatch")
            location["read_mode"] = "current_only"
    ...
    def test_exact_adamw_rng_resume_and_group_names(self):
        ...
        other = VisualDifferentialMemoryV12(self.visual_config)
