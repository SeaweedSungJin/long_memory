...
## Local long-memory research extension

이 작업 공간에 추가한 **2-stage long-memory 학습 코드**는 원본 HAMLET과 별도다.
[실행법·모듈 설명·그래프 해석](docs/LONG_MEMORY_TRAINING.md)과
[검증 기록](docs/LONG_MEMORY_VALIDATION.md)을 참고한다.
[실시간 TensorBoard 대시보드](docs/LONG_MEMORY_LIVE.md)로 학습 중 곡선을 볼 수 있다.
[현재 체크포인트의 RoboMME 성공률 비교](docs/LONG_MEMORY_EVAL.md)는 원본 HAMLET,
Stage 1, Stage 2를 같은 episode와 seed에서 평가하는 별도 실행 경로다.
[모델 구조·기존 실험 기록](docs/LONG_MEMORY_ARCHITECTURE.md)과
[개선된 Stage 2 v2 실행법](docs/LONG_MEMORY_STAGE2_V2.md)을 별도로 보존한다.
새 **action-value v3**는 [구조·학습·TensorBoard·재개 안내](docs/LONG_MEMORY_V3_TRAINING.md)와
[RoboMME 비교 평가 안내](docs/LONG_MEMORY_V3_EVAL.md)를 참고한다.
V3는 역할·시간 기반 reader와 KEEP/APPEND/REPLACE writer를 사용하는 별도 checkpoint 형식이다.
[변경 전 소스 보존본](docs/archives/pre_action_value_v3_ucwVvb/README.md)도 남겼으며, 기존 v1/v2는 그대로 사용할 수 있다.
기존 HAMLET은 동결하고 새 memory만 offline 학습하며, 별도 adapter로 simulator 평가한다.
저자의 공식 학습 절차나 simulator 성능 결과와 혼동하지 않는다.
